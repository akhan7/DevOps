...
app.get('/spawn/:port', function(req, res) {
	{
		...
		client.lpush("proxyQueue", "http://127.0.0.1:" + req.params.port);
	}
})
app.get('/listservers', function(req, res) {
	{
		client.lrange('proxyQueue', 0, -1, function(err, servers){
			res.send(servers);
		});		
		client.llen('proxyQueue', function(err, length){
			console.log(length);
		});
	}
})
app.get('/destroy', function(req, res) {
	{
		...
		client.llen('proxyQueue', function(err, length){
			...
			client.lindex('proxyQueue', rand, function(err, ind){
				 client.lrem('proxyQueue', 1, ind);
...
var server = app.listen(3000, function () {
  ...
  client.del('proxyQueue', function(err, reply) {
	...
	client.lpush('proxyQueue', url1, function(err, reply) {
...
client.lpush("proxyQueue", "http://127.0.0.1:3000");
